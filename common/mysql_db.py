...
dbconfig = ReadConfig().get_db_config()
...
class SmartMySQL:
    ...
    def __init__(self, dbconfig):
        try:
            self._dbconfig = db_config
            self.my_logger = MyLogger('../logs/mysql-' + time.strftime("%Y-%m-%d-%H-%M-%S") + '.log')
            self.check_dbconfig(db_config)
        ...
        self.my_logger.info("Database init success and connect it")
    ...
    def check_dbconfig(self, dbconfig):
        ...
        if type(dbconfig) is not dict:
            ...
            self.my_logger.error("dbconfig is not dict")
            ...
            for key in ['host', 'port', 'user', 'password', 'db']:
                if key not in dbconfig:
                    ...
                    msg = "dbconfig error: do not have %s" % key
...
#     db_config = ReadConfig().get_db_config()
#     print(db_config)
#     logger = SmartMySQL(db_config)
#     print(logger)
